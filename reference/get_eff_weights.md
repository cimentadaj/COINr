...
When `coin` inherits from `unbalanced_coin`, `get_eff_weights()` removes
any placeholder codes that were injected to balance the hierarchy before
delegating to the standard implementation. Placeholder nodes are also
stripped from the returned data frame or coin output so that only the
genuine indicators and aggregates appear.
