...
When `coin` inherits from `unbalanced_coin`, `get_PCA()` delegates to
the balanced implementation while filtering out placeholder helper nodes
inserted during balancing. Any placeholder codes are pruned from the
returned weight tables and stored weight sets so that only genuine
indicators and aggregates are exposed.
