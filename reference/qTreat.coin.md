...
- x:
  ...
  For `qTreat.coin()`, a coin object; for `qTreat.unbalanced_coin()` an
  `unbalanced_coin` object.
...
- f2:
  ...
  within limits. Default `"boxcox_auto"`.
...
A coin (or `unbalanced_coin`) with treated data set at `.$Data$Treated`.
...
- `qTreat(unbalanced_coin)`: Wrapper that keeps the unbalanced hierarchy
  while applying the quick treatment defaults.
