...
When the input inherits from `unbalanced_coin`, `get_opt_weights()`
delegates to the balanced implementation, then removes placeholder
helper nodes from the outward results so that only genuine indicators
