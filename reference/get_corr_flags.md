...
- ...:
  ...
  Additional arguments passed to `get_corr_flags.coin()`.
...
When `coin` inherits from `unbalanced_coin`, `get_corr_flags()`
delegates to the balanced implementation while stripping any placeholder
nodes from the metadata and returned rows. As a result, only genuine
indicators appear in the flagged pairs, mirroring the behaviour of
[`get_corr()`](https://bluefoxr.github.io/COINr/reference/get_corr.md)
