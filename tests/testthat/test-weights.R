...
test_that("get_opt_weights.unbalanced_coin keeps placeholders in output", {
  # Placeholders are intentionally kept in output because they represent real
  # weights that affect how indicators contribute to the Index
  ...
  # Placeholders should be present in output (intentional design)
  if(is.data.frame(opt_list$WeightsOpt) && length(ph_codes) > 0){
    expect_true(any(opt_list$WeightsOpt$iCode %in% ph_codes))
  }
